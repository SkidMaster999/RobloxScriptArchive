...
--NP"C:\Users\Yash\message_11_2.txt}?)Jr"qev
...
local function Bring(plr, tool, cframe, second) -- thanks fate for teaching me the humanoid cloning & deletion (ive never touched it before) (should probably be noted on the clonehumanoid func)
    ...
    task.spawn(pcall, firetouchinterest, tool:FindFirstChild("Handle"), plr.Character.PrimaryPart, ffalse)
    Goto(cframe)
    task.wait(math.clamp((second or 0) + (GetPing() / ((cframe.p - saved.p).magnitude > 5 and 2 or 4) * 2), 0, 4.5))
    togs.Noclip = oldnc
    Respawn(nil, saved)
end
    ...
    do
        ...
        Bar.Changed:Connect(function()
            ...
            for _, v in next, commands do
                ...
                --[[if v.Command:lower() == cmd:lower() then
                    cmdtodisplay 
                end]]
            ...
            getgc()[i] = function(bullets, istaser)
                if togs.AntiCrash then
                    for i,v in pairs(bullets) do
                        if type(v) ~= "table" or not v.RayObject or not v.Cframe or not v.Distance or v.Distance > 800 or v.Distance == 0 or (v.RayObject.Origin - v.RayObject.Direction).magnitude > 800 then
                            return
                        end
                    end
                end
...
do
    --[[workspace:WaitForChild("Remote"):WaitForChild("toggleSiren"):FireServer({
        Part1 = {Weld_Part = true, l = workspace:WaitForChild("Remote"):WaitForChild("toggleSiren"):WaitForChild("sirenToggleScript")},
        Part2 = {Weld_Part = true, l = {}},
        Part3 = {Weld_Part = true, l = {}},
        Part4 = {Weld_Part = true, l = {}},
        isOn = {Value = false},
        Speaker = {Weld_Part = true, Sound = workspace:WaitForChild("Prison_guardspawn"):WaitForChild("spawn"):WaitForChild("Sound")}
    })]]

    local eloopkilltable = {}
    local ELoopkill = {Custom = false, Inmates = false, Guards = false, Neutral = false, Criminals = false}
    local function EKill(plrs)
        for i,v in next, plrs do
            if v.Character and v ~= lp then
                workspace.Remote.toggleSiren:FireServer({
                    Part1 = {Weld_Part = true, l = v.Character:WaitForChild("Torso"):WaitForChild("Neck")},
                    isOn = {Value = false},
                    Speaker = {Weld_Part = true, Sound = v.Character:WaitForChild("Head"):WaitForChild("punchSound")}
                })
            end
        end
    end

    local function EKillCheck(plr)
        plr.CharacterAdded:Connect(function()
            if table.find(eloopkilltable, plr.Name) or (ELoopkill.Custom and not plr.Team) or ELoopkill[tostring(plr.Team)] and not table.find(togs.Whitelist, plr.Name) then
                EKill{plr}
            end
        end)
    end
    ...
    sv.Players.PlayerAdded:Connect(EKillCheck)

    AddCommand("epicloopkill", {"eloopkill", "elk"}, "<players | teams...>\nLoopkill, but troller", true, true, function(plr, ...)
        for i,v in pairs({...}) do
            local t = GetTeam(v) or GetPlayer(v)
            if t == lp then continue end
            if not t then continue end
            ...
            table.insert(eloopkilltable, t.Name)
        end
    end)

    AddCommand("unepicloopkill", {"uneloopkill", "unelk"}, "<players | teams...>\nUnloopkill, but troller", true, true, function(plr, ...)
        for i,v in pairs({...}) do
            local t = GetTeam(v) or GetPlayer(v)
            if not t then continue end
            ...
            local f = table.find(eloopkilltable, t.Name)

            if f then
                table.remove(eloopkilltable, f)
            end
        end
    end)

    AddCommand("epickill", {"ekill", "ek"}, "<players | teams...>\nKill, but troller", true, true, function(plr, ...)
        local kt = {}
    ...
    AddCommand("disable", {"d"}, "<scriptpath>\ndisables a certain script in Remotes", true, false, function(plr, ...)
        for i,v in next, {...} do
            if workspace.Remote:FindFirstChild(v) and workspace.Remote[v]:FindFirstChildOfClass("Script") then
                workspace.Remote.toggleSiren:FireServer({
                    Part1 = {Weld_Part = true, l = workspace.Remote[v]:FindFirstChildOfClass("Script")},
                    isOn = {Value = false}, 
                    Speaker = {Weld_Part = true, Sound = lp.Character:WaitForChild("Head"):WaitForChild("punchSound")}
                })
            end
        end
    end)

    AddCommand("kick", nil, "<player>\nKicks player", true, true, function(plr, ...) 
        local kplr = GetPlayer(...)

        if kplr ~= lp and kplr ~= nil then
            Team(kplr.TeamColor.Name)
            task.wait(1)
            game:GetService("Workspace").Remote.Legacy.votekick:FireServer("Start", kplr)
            task.wait(.3)

            for i = 1, 24 do
                game:GetService("Workspace").Remote.Legacy.votekick:FireServer("Vote")
            end

            kplr.AncestryChanged:Once(function() 
                Note(("Sucessfully kicked %s!"):format(kplr.Name))
                task.wait(40)
                Note("Kick is now ready.")
            end)
        end
    end)
end
